#!/usr/bin/env Rscript
# CRAPome 数据清洗：复用 HGSCore/统一准备层逻辑，删除 MaxQuant 常见伪阳性标记。
...
if (file.exists(unified_cleaned)) {
  ...
  stop("找不到输入文件。请提供 --input= 或先运行统一数据准备层。")
}

write_tsv(dat, output_file)
writeLines(c("CRAPome 数据清洗记录", paste("时间:", Sys.time()), paste("来源:", source_type), paste("原始行数:", n_original), paste("清洗后行数:", n_cleaned), paste("删除行数:", n_original - n_cleaned)), log_file, useBytes = TRUE)
cat("输出: ", output_file, "\n", sep = "")
